...
document.addEventListener("DOMContentLoaded", function() {
  document.getElementById("transactions").innerHTML = renderTransactions(
    fullTransactionData
  );
  ...
  document
    ...
    .addEventListener("input", function(e) {
      ...
      var filteredData = fullTransactionData.filter(function(transaction) {
        var foundInName =
          transaction.name.toLowerCase().indexOf(searchString) > -1;
        var foundInFor =
          transaction.for.toLowerCase().indexOf(searchString) > -1;
        var foundInDate =
          transaction.date.toLowerCase().indexOf(searchString) > -1;
        var foundInAmount =
          transaction.amount.toLowerCase().indexOf(searchString) > -1;
